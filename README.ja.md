...
| `beat_mode_start(motion_intensity?, color?, duration_sec?)` | gateway-only の beat mode を開始する。既存の `listen` wire path でデバイス周辺音を継続キャプチャし、gateway 側で BPM を推定して、free-running の beat 同期 head sway とベースリング LED flash を動かす。beat mode は firmware の raw capture profile を要求し、音楽が発話向け AFE 抑制を bypass する。有効中の `listen()` は排他で失敗し、`say()` は割り込み可能で、発話後に beat mode が listening を再開する。Opus 復号用に `[stt]` extra が必要 | ✅ |
